...
{
AdmittanceController::AdmittanceController() : controller_interface::ControllerInterface() {}

CallbackReturn AdmittanceController::on_init()
{
  try
  {
    // TODO(destogl): use variables as parameters
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  auto get_bool_param_and_error_if_empty = [&](bool & parameter, const char * parameter_name) {
    parameter = get_node()->get_parameter(parameter_name).get_value<bool>();
    return false;  // TODO(destogl): how to check "if_empty" for bool?
  };

  auto get_double_param_and_error_if_empty = [&](double & parameter, const char * parameter_name) {
  ...
  auto get_double_params_and_error_if_both_empty_or_set =
    [&](
      double & parameter, const char * parameter_name, double & parameter2,
      const char * parameter_name2) {
      parameter = get_node()->get_parameter(parameter_name).get_value<double>();
      parameter2 = get_node()->get_parameter(parameter_name2).get_value<double>();
      if (std::isnan(parameter) == std::isnan(parameter2))
      {
        RCLCPP_ERROR(
          get_node()->get_logger(), "'%s' and '%s' parameters were both '%s'set", parameter_name,
          parameter_name2, (std::isnan(parameter) ? "not " : ""));
        return true;
      }
      return false;
    };
  ...
  if (
    ...
    get_bool_param_and_error_if_empty(
      use_joint_commands_as_input_, "use_joint_commands_as_input") ||
    ...
    get_bool_param_and_error_if_empty(
      admittance_->selected_axes_[0], "admittance.selected_axes.x") ||
    get_bool_param_and_error_if_empty(
      admittance_->selected_axes_[1], "admittance.selected_axes.y") ||
    get_bool_param_and_error_if_empty(
      admittance_->selected_axes_[2], "admittance.selected_axes.z") ||
    get_bool_param_and_error_if_empty(
      admittance_->selected_axes_[3], "admittance.selected_axes.rx") ||
    get_bool_param_and_error_if_empty(
      admittance_->selected_axes_[4], "admittance.selected_axes.ry") ||
    get_bool_param_and_error_if_empty(
      admittance_->selected_axes_[5], "admittance.selected_axes.rz") ||
    ...
    get_double_params_and_error_if_both_empty_or_set(
      admittance_->damping_[0], "admittance.damping.x", admittance_->damping_ratio_[0],
      "admittance.damping_ratio.x") ||
    get_double_params_and_error_if_both_empty_or_set(
      admittance_->damping_[1], "admittance.damping.y", admittance_->damping_ratio_[1],
      "admittance.damping_ratio.y") ||
    get_double_params_and_error_if_both_empty_or_set(
      admittance_->damping_[2], "admittance.damping.z", admittance_->damping_ratio_[2],
      "admittance.damping_ratio.z") ||
    get_double_params_and_error_if_both_empty_or_set(
      admittance_->damping_[3], "admittance.damping.rx", admittance_->damping_ratio_[3],
      "admittance.damping_ratio.rx") ||
    get_double_params_and_error_if_both_empty_or_set(
      admittance_->damping_[4], "admittance.damping.ry", admittance_->damping_ratio_[4],
      "admittance.damping_ratio.ry") ||
    get_double_params_and_error_if_both_empty_or_set(
      admittance_->damping_[5], "admittance.damping.rz", admittance_->damping_ratio_[5],
  ...
  if (!admittance_->filter_chain_->configure(
        "input_wrench_filter_chain", get_node()->get_node_logging_interface(),
  ...
  for (const auto & interface : command_interface_types_)
  {
    ...
    {
      RCLCPP_ERROR_STREAM(
        get_node()->get_logger(), "Command interface type '" << interface << "' not allowed!");
      return CallbackReturn::ERROR;
    }
  }
  ...
  for (const auto & interface : state_interface_types_)
  {
    ...
    {
      RCLCPP_ERROR_STREAM(
        get_node()->get_logger(), "State interface type '" << interface << "' not allowed!");
      return CallbackReturn::ERROR;
    }
  }
  ...
  {
    RCLCPP_ERROR_STREAM(
      get_node()->get_logger(), "State interface type '"
                                  << std::string(hardware_interface::HW_IF_POSITION)
                                  << "' has to be always present allowed!");
    return CallbackReturn::ERROR;
  }
  ...
  {
    auto callback_input_force = [&](const std::shared_ptr<ControllerCommandWrenchMsg> msg) -> void {
  ...
  auto callback_input_joint = [&](const std::shared_ptr<ControllerCommandJointMsg> msg) -> void {
  ...
  auto callback_input_pose = [&](const std::shared_ptr<ControllerCommandPoseMsg> msg) -> void {
...
CallbackReturn AdmittanceController::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  for (const auto & interface : command_interface_types_)
  {
    ...
    {
      RCLCPP_ERROR(
        node_->get_logger(), "Expected %zu '%s' command interfaces, got %zu.", num_joints,
  ...
  for (const auto & interface : state_interface_types_)
  {
    ...
    {
      RCLCPP_ERROR(
        node_->get_logger(), "Expected %zu '%s' state interfaces, got %zu.", num_joints,
  ...
  std::shared_ptr<ControllerCommandWrenchMsg> msg_wrench =
    std::make_shared<ControllerCommandWrenchMsg>();
  msg_wrench->header.frame_id = admittance_->control_frame_;
  input_wrench_command_.writeFromNonRT(msg_wrench);

  std::shared_ptr<ControllerCommandJointMsg> msg_joint =
    std::make_shared<ControllerCommandJointMsg>();
  ...
  {
    RCLCPP_ERROR(
      node_->get_logger(), "Can not find transform from '%s' to '%s' needed in the update loop",
...
controller_interface::return_type AdmittanceController::update()
{
  ...
  if (use_joint_commands_as_input_)
  {
    ...
    if ((*input_joint_cmd)->points[0].positions.empty())
    {
      ...
      {
        joint_deltas[index] =
          (*input_joint_cmd)->points[0].velocities[index] * duration_since_last_call.seconds();
      }
    }
    else
    {
      for (auto index = 0u; index < num_joints; ++index)
      {
        ...
        joint_deltas[index] = angles::shortest_angular_distance(
          current_joint_states.positions[index], (*input_joint_cmd)->points[0].positions[index]);
      }
    }

    admittance_->update(
      current_joint_states, ft_values, joint_deltas, duration_since_last_call,
      desired_joint_states);
  }
  else
  {
    admittance_->update(
      current_joint_states, ft_values, **input_pose_cmd, duration_since_last_call,
      desired_joint_states);
  }
  //   }
  previous_time_ = get_node()->now();

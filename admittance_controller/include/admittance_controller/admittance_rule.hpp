...
template <typename Type>
void convert_message_to_array(const geometry_msgs::msg::PoseStamped & msg, Type & vector_out)
{
...
template <typename Type>
void convert_message_to_array(const geometry_msgs::msg::Wrench & msg, Type & vector_out)
{
...
template <typename Type>
void convert_message_to_array(const geometry_msgs::msg::WrenchStamped & msg, Type & vector_out)
{
...
public:
  ...
  bool check_if_parameters_are_valid() override
  {
    ...
    for (auto i = 0ul; i < 6; ++i)
    {
      if (bool_parameters_[offset_index_bool + i].second)
      {
        ...
        index = i + 12;
        if (
          std::isnan(double_parameters_[index].second) &&
          std::isnan(double_parameters_[index + 6].second))
        {
          RCUTILS_LOG_ERROR_NAMED(
            logger_name_.c_str(), "Either parameter '%s' of '%s' has to be set",
            double_parameters_[index].first.name.c_str(),
    ...
    for (auto i = 0ul; i < damping_ratio_.size(); ++i)
    {
      ...
      else
      {
        RCUTILS_LOG_DEBUG_NAMED(
          logger_name_.c_str(), "Damping ratio for axis %zu not used because it is NaN.", i);
      }
    }
  }
  ...
  void update_storage() override
  {
    ...
    RCUTILS_LOG_INFO_NAMED(logger_name_.c_str(), "IK Base frame: %s", ik_base_frame_.c_str());
    ik_group_name_ = string_parameters_[1].second;
    RCUTILS_LOG_INFO_NAMED(logger_name_.c_str(), "IK group name frame: %s", ik_group_name_.c_str());
    ...
    RCUTILS_LOG_INFO_NAMED(
      logger_name_.c_str(), "Using open loop: %s", (open_loop_control_ ? "True" : "False"));

    for (auto i = 0ul; i < 6; ++i)
    {
      selected_axes_[i] =
        bool_parameters_[i + 1].second;  // +1 because there is already one parameter
      RCUTILS_LOG_INFO_NAMED(
        logger_name_.c_str(), "Axis %zu is %sselected", i, (selected_axes_[i] ? "" : "not "));

      mass_[i] = double_parameters_[i].second;
      RCUTILS_LOG_INFO_NAMED(logger_name_.c_str(), "Mass for the axis %zu is %e", i, mass_[i]);
      stiffness_[i] = double_parameters_[i + 6].second;
      RCUTILS_LOG_INFO_NAMED(
        logger_name_.c_str(), "Stiffness for the axis %zu is %e", i, stiffness_[i]);
      damping_[i] = double_parameters_[i + 12].second;
      RCUTILS_LOG_INFO_NAMED(
        logger_name_.c_str(), "Damping for the axis %zu is %e", i, damping_[i]);
      damping_ratio_[i] = double_parameters_[i + 18].second;
      RCUTILS_LOG_INFO_NAMED(
        logger_name_.c_str(), "Damping_ratio for the axis %zu is %e", i, damping_ratio_[i]);
    }
...
public:
  ...
  controller_interface::return_type update(
    ...
    const geometry_msgs::msg::PoseStamped & reference_pose, const rclcpp::Duration & period,
    trajectory_msgs::msg::JointTrajectoryPoint & desired_joint_states);

  controller_interface::return_type update(
    const trajectory_msgs::msg::JointTrajectoryPoint & current_joint_state,
    const geometry_msgs::msg::Wrench & measured_wrench,
    const std::array<double, 6> & reference_joint_deltas, const rclcpp::Duration & period,
    trajectory_msgs::msg::JointTrajectoryPoint & desired_joint_states);
  ...
  controller_interface::return_type update(
    ...
    const geometry_msgs::msg::WrenchStamped & reference_force, const rclcpp::Duration & period,
    trajectory_msgs::msg::JointTrajectoryPoint & desired_joint_states);

  controller_interface::return_type get_controller_state(
    control_msgs::msg::AdmittanceControllerState & state_message);

  controller_interface::return_type get_pose_of_control_frame_in_base_frame(
...
protected:
  void process_wrench_measurements(const geometry_msgs::msg::Wrench & measured_wrench);

  /**
   * All values are in he controller frame
   */
  void calculate_admittance_rule(
    const std::array<double, 6> & measured_wrench, const std::array<double, 6> & pose_error,
    const rclcpp::Duration & period, std::array<double, 6> & desired_relative_pose);

  controller_interface::return_type calculate_desired_joint_state(
    const trajectory_msgs::msg::JointTrajectoryPoint & current_joint_state,
    const std::array<double, 6> & relative_pose, const rclcpp::Duration & period,
...
private:
  ...
  controller_interface::return_type transform_to_control_frame(
    const MsgType & message_in, MsgType & message_out)
  {
    return transform_to_frame(message_in, message_out, parameters_.control_frame_);
  }

  template <typename MsgType>
  controller_interface::return_type transform_to_ik_base_frame(
    const MsgType & message_in, MsgType & message_out)
  {
  ...
  template <typename MsgType>
  controller_interface::return_type transform_to_frame(
    const MsgType & message_in, MsgType & message_out, const std::string & frame)
  {
  ...
  template <typename MsgType>
  controller_interface::return_type transform_relative_to_control_frame(
    const MsgType & message_in, MsgType & message_out)
  {
  ...
  template <typename MsgType>
  controller_interface::return_type transform_relative_to_ik_base_frame(
    const MsgType & message_in, MsgType & message_out)
  {
   ...
   */
  template <typename MsgType>
  controller_interface::return_type transform_relative_to_frame(
    const MsgType & message_in, MsgType & message_out, const std::string & frame)
  {
  ...
  template <typename Type>
  void direct_transform(const Type & input, const tf2::Transform & transform, Type & output)
  {
